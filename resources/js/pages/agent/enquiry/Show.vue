...
const isCurrentUserSender = (message: Message) => {
  return message.sender_id === props.enquiry.agent_id;
};

const resolveEnquiryForm = useForm();

const handleResolveEnquiry = async () => {
    resolveEnquiryForm.post(enquiries.resolved.url(props.enquiry.id), {
        preserveScroll: true,
    });
};
    ...
    <AgentLayout>
        ...
        <div class="border-b bg-background sticky top-0 z-10">
            <div class="flex items-center gap-4 p-4">
                ...
                <div v-if="getEnquiryStatusValue(props.enquiry.status).value != 3">
                    <Form @submit.prevent="handleResolveEnquiry">
                        <Button variant="default" type="submit" size="sm" :disabled="resolveEnquiryForm.processing || getEnquiryStatusValue(props.enquiry.status).value == 3">
                            <CheckCircle2 class="h-4 w-4" /> Make Resolved
                        </Button>
                    </Form>
                </div>
                
                <Badge :class="getEnquiryStatusValue(props.enquiry.status).badgeClass" class="py-2 px-5">
                    {{ getEnquiryStatusValue(props.enquiry.status).label }}
                </Badge>
            </div>
        </div>
        ...
        <div class="border-t bg-background p-4 sticky bottom-0">
            ...
            <Textarea
                ...
                :disabled="messageForm.processing || getEnquiryStatusValue(props.enquiry.status).value == 3"
                @keydown.enter.ctrl.exact="handleSendMessage"
            />
            <Button
                type="submit"
                size="icon"
                :disabled="!messageForm.body.trim() || messageForm.processing || getEnquiryStatusValue(props.enquiry.status).value == 3"
                class="flex-shrink-0"
            >
                <Send class="h-4 w-4" />
            </Button>
